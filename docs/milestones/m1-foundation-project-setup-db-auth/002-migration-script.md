...
- [ ] Migration runner (e.g. `scripts/migrate.ts` or `scripts/migrate.js`) that executes `schema/01_extensions_enums.sql` through `schema/10_triggers.sql`
- [ ] Include `schema/06b_user_connections.sql` after `06_user_problem_selections.sql` (per db-schema.md)
...
- Schema files: `01_extensions_enums.sql` → `10_triggers.sql` (see `schema/` directory)
